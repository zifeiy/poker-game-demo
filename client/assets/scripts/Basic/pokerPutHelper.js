...
7: 红十炸
*/
pokerPutHelper.judgeType = function (cards) {
    cards.sort(this.normalCmp);
    ...
    if (cards.length == 1) {
        ...
        var y = arr[1];
        var z = arr[2];
        console.log('cards.len=3:' + JSON.stringify(cards) + ': x=' + x + ',y=' + y + ',z=' + z);
 ...
 * false: 不能跟 
*/
pokerPutHelper.checkCanFollow = function (oldCards, newCards) {
    var type1 = this.judgeType(oldCards);
    var type2 = this.judgeType(newCards);
    console.log('\tjudgeType: ' + JSON.stringify(oldCards) + ':' + type1 + ', ' + JSON.stringify(newCards) + ':' + type2);
    if (oldCards.length == 0 && type2 != -1) {
        return true;
    }
...
pokerPutHelper.getCardList = function (cards, type, len) {
    var resList = [];
    if (type == 1) {
...
pokerPutHelper._getTipCardsByType = function (putCards, myCards, type) {
...
pokerPutHelper._getAllTipCardsByType = function (putCards, myCards, type) {
    var resList = [];
    var cardsList = this.getCardList(myCards, type, putCards.length);
    for (var i = 0; i < cardsList.length; i ++) {
        var cards = cardsList[i];
...
pokerPutHelper.getTipCards = function (putCards, myCards) {
...
pokerPutHelper.getAllTipCards = function (putCards, myCards) {
    var type = this.judgeType(putCards);
    var resList = [];
    if (putCards.length == 0) {
        var res;
...
pokerPutHelper.cardCmp = function (x, y) {      // 从小到达排序
    var xx = x % 100;
    var yy = y % 100;
...
pokerPutHelper.test = function (cards) {
...
pokerPutHelper.test2 = function () {
    ...
    console.log('putCard type = ' + this.judgeType(putCards));
...
pokerPutHelper.normalCmp = function (a, b) {
    return a - b;
}

module.exports = pokerPutHelper;
...
// }
// console.log('type: ' + pokerPutHelper.judgeType(arr) + ' ; ' + JSON.stringify(arr));

var oldCards = [103];
var newCards = [101];
var res = pokerPutHelper.checkCanFollow(oldCards, newCards);
console.log('checkCanFollow: ' + res + ' : old:' + JSON.stringify(oldCards) + ', new:' + JSON.stringify(newCards));

// setInterval(()=>{
//     pokerPutHelper.test2();
// }, 1000);
